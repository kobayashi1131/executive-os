---
name: "source-command-initial-setup"
description: "Migrated source command `initial-setup`"
---

# source-command-initial-setup

Use this skill when the user asks to run the migrated source command `initial-setup`.
...
`.Codex / work / data` の役割を確認し、会社メモリ、作業場所、GitHub Actionsの確認まで案内する。
...
```bash
test -d .Codex && test -d work && test -d data && echo "OK: core directories exist"
```

### 2. 実行権限付与

```bash
chmod +x data/tools/scripts/*.sh .Codex/hooks/*.sh
```
